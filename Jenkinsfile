pipeline {
    ...
    stages {
        ...
        stage ('Updating Manifest') {
            steps {
                script {
                sh "curl -v -k —user Scion_Scope:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'contact-type: application/x-ww-form-urlencoded'  —data  'IMAGE_TAG=${IMAGE_TAG}' 'http://100.25.22.249:8080/job/second-job/buildWithParameters?token=Authentication-Token' "
                }
            }
        }
    }
